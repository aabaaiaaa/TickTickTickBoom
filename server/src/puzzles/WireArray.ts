...
const WIRE_COLORS = ['red', 'blue', 'yellow', 'white', 'black', 'green'] as const;
...
export interface WireArrayView {
    wires: Wire[];
    cutWires: number[]; // Indices of wires that have been cut
}

interface WireArraySolution {
    correctCuts: number[]; // Wire positions to cut in order
}
...
const puzzleSolutions = new Map<string, WireArraySolution>();

export function generateWireArray(
    ...
    const wires: Wire[] = [];
    const usedSymbols = new Set<WireSymbol>();
    ...
    const solution: WireArraySolution = { correctCuts };
...
function determineCorrectCuts(wires: Wire[], serialNumber: string): number[] {
...
export function validateWireArray(puzzle: PuzzleInstance, action: unknown): ValidationResult {
    const view = puzzle.defuserView as WireArrayView;
    const solution = puzzle.solution as WireArraySolution;
    const actionData = action as { type?: string; wireIndex: number };
