...
const DISPLAY_TYPES = ['number', 'color'] as const;
type DisplayType = typeof DISPLAY_TYPES[number];

const BUTTON_COLORS = ['red', 'blue', 'green', 'yellow'] as const;
type ButtonColor = typeof BUTTON_COLORS[number];

export interface SequenceMemoryView {
    display: string; // Current display value (number 1-4 or color word)
    displayType: DisplayType;
    buttons: ButtonColor[]; // Fixed positions: [position0, position1, position2, position3]
    currentStage: number;
    totalStages: number;
    stageHistory: Array<{ position: number; color: ButtonColor }>; // What was pressed each stage
}
...
const BUTTON_POSITIONS: ButtonColor[] = ['red', 'blue', 'green', 'yellow'];

export function generateSequenceMemory(
    ...
    // Generate stages
    const stages: SequenceMemorySolution['stages'] = [];

    for (let i = 0; i < totalStages; i++) {
        const displayType: DisplayType = Math.random() > 0.5 ? 'number' : 'color';
        let display: string;
        ...
        if (displayType === 'number') {
            ...
            correctPosition = BUTTON_POSITIONS.indexOf(colorWord as ButtonColor);
        }
...
export function validateSequenceMemory(puzzle: PuzzleInstance, action: unknown): ValidationResult {
    const view = puzzle.defuserView as SequenceMemoryView;
    const solution = puzzle.solution as SequenceMemorySolution;
    const actionData = action as { position: number };
    ...
    if (stageSolution.displayType === 'number') {
        ...
        // Color display rules
        const displayColor = stageSolution.display.toLowerCase() as ButtonColor;
        if (view.currentStage === 1) {
