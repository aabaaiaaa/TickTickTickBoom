...
const CAPACITOR_COLORS = ['red', 'yellow', 'blue', 'green', 'purple'] as const;
...
export interface Capacitor {
    ...
    isCritical: boolean; // Blinking warning
}
...
export function generateCapacitorBank(
    ...
    const targetVoltages: Record<string, { min: number; max: number }> = {};
...
export function validateCapacitorBank(puzzle: PuzzleInstance, action: unknown): ValidationResult {
    const view = puzzle.defuserView as CapacitorBankView;
    ...
    if (actionType === 'discharge') {
        ...
        const hasUnsafe = view.capacitors.some(c => {
            if (c.isCritical && c.colorBand === 'red' && c.voltage > 50) return true;
            if (c.isCritical && c.colorBand === 'blue' && (c.voltage < 55 || c.voltage > 65)) return true;
            if (c.voltage < 30 || c.voltage > 80) return true;
            return false;
        });
