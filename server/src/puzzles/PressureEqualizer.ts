...
const POSITIONS: SliderPosition[] = ['A', 'B', 'C', 'D', 'E'];

export interface Slider {
    id: number;
    position: SliderPosition;
    pressure: number; // Derived from position (A=2, B=4, C=6, D=8, E=10)
...
function positionToPressure(pos: SliderPosition): number {
    return (POSITIONS.indexOf(pos) + 1) * 2;
}

function calculateIndicator(slider: Slider, systemPressure: number): IndicatorStatus {
    if (slider.isLocked) return 'red';
    if (slider.pressure >= 4 && slider.pressure <= 8) return 'green';
    return 'yellow';
}

export function generatePressureEqualizer(
...
export function validatePressureEqualizer(puzzle: PuzzleInstance, action: unknown): ValidationResult {
    const view = puzzle.defuserView as PressureEqualizerView;
    ...
    if (actionType === 'move' || actionType === 'adjust-slider') {
        ...
        if (actionData.position && POSITIONS.includes(actionData.position as SliderPosition)) {
            slider.position = actionData.position as SliderPosition;
    ...
    if (actionType === 'confirm' || actionType === 'confirm-pressure') {
        ...
        if (actionData.positions) {
            for (let i = 0; i < view.sliders.length && i < actionData.positions.length; i++) {
                const pos = actionData.positions[i] as SliderPosition;
        ...
        for (let i = 0; i < view.sliders.length - 1; i++) {
            const diff = Math.abs(
                POSITIONS.indexOf(view.sliders[i].position) -
