...
export class RoomManager {
    rooms = new Map();
    playerRooms = new Map(); // socketId -> roomCode
    gameTimers = new Map();
    ...
    setDifficulty(socketId, difficulty) {
        const room = this.getRoomBySocket(socketId);
        if (!room || room.phase !== 'lobby')
            return null;
    ...
    handleTakeover(socketId) {
        const room = this.getRoomBySocket(socketId);
        if (!room || room.phase !== 'paused')
            return null;
        const player = room.players.find(p => p.socketId === socketId);
        if (!player || player.role !== 'reader')
            return null;
