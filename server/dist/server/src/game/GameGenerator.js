...
export function generateGame(difficulty, puzzleCount) {
    ...
    const puzzles = selectedPuzzles.map((generator, index) => {
...
function selectPuzzles(difficulty, count) {
...
function logPuzzleSolutions(puzzles, serialNumber, indicators) {
    console.log('\n' + '='.repeat(60));
    ...
    puzzles.forEach((puzzle, index) => {
        ...
        switch (puzzle.type) {
            case 'wire-array': {
                const correctCuts = solution?.correctCuts || [];
                const wires = view?.wires || [];
                ...
                correctCuts.forEach(pos => {
                    ...
                    if (wire)
                        console.log(`    - Position ${pos}: ${wire.color} wire with symbol ${wire.symbol}`);
                });
                break;
            }
            case 'button-matrix': {
            ...
            case 'simon-signals': {
                const fullSequence = solution?.fullSequence || view?.sequence || [];
                const translationKey = solution?.translationKey;
                const hasVowel = view?.hasVowelInSerial;
            ...
            case 'capacitor-bank': {
                const targetValues = solution?.targetValues || [];
            ...
            case 'mechanical-switches': {
                const targetStates = solution?.targetStates || [];
